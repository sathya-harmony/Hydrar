                    ...
                    if xp == 0:
                        ...
                        for i in range(len(level)):
                            if lvl == levelnum[i]:
                                ...
                                embed = discord.Embed(
                                    description=
                                    f"{message.author.mention} you have gotten the role **{level[i]}**!!"
                                )
                                embed.set_thumbnail(
        ...
        if ctx.channel.id == bot_channel:
            ...
            if stats is None:
                await ctx.channel.send("You haven't sent any messaged to Level up!")
                 ...
                 embed.add_field(name = "**XP**", value = f"{xp}/{int(100*lvl)}", inline=False)
                 embed.add_field(name = "**Rank**", value = f"{rank}/{ctx.guild.member_count}", inline=False)
                 embed.add_field(name ="Progress Bar", value = boxes * "🟦"+(20-boxes)*"⬜", inline = False) 
